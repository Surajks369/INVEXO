              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <form action="{{ route('subscriptions.update', $subscription->id) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                      <label>Price</label>
                      <input type="number" step="0.01" name="price" class="form-control" value="{{ old('price', $subscription->price) }}" required>
                    </div>
                    <div class="form-group mb-3">
                      <label>Duration</label>
                      <select name="duration" class="form-control" required>
                        <option value="WEEKLY" {{ old('duration', $subscription->duration) == 'WEEKLY' ? 'selected' : '' }}>WEEKLY</option>
                        <option value="MONTHLY" {{ old('duration', $subscription->duration) == 'MONTHLY' ? 'selected' : '' }}>MONTHLY</option>
                        <option value="YEARLY" {{ old('duration', $subscription->duration) == 'YEARLY' ? 'selected' : '' }}>YEARLY</option>
                      </select>
                    </div>
